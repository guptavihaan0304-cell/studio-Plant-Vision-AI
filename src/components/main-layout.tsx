          ...
          <nav className="mx-auto mb-4 max-w-md w-[calc(100%-2rem)] glassmorphic-panel p-2 rounded-full">
            <div className="flex justify-around items-center">
              {navItems.map((item) => {
                ...
                return (
                  <Link href={item.href} key={item.href} className="flex flex-col items-center justify-center gap-1 text-muted-foreground w-16 h-16 rounded-full transition-all duration-150 ease-in-out group">
                    <div className={cn(
                      "relative flex items-center justify-center w-12 h-12 rounded-full transition-all duration-150 ease-in-out",
